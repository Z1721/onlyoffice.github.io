...
(function (window, undefined) {
  ...
  window.Asc.plugin.ie_channel_check = function (e) {
    ...
    if (uagent.indexOf("msie") > -1 || uagent.indexOf("trident") > -1) {
      if (e.ports && e.ports[0]) this.ie_channel = e.ports[0];
    }
  };
  ...
  window.Asc.scope.prototype = {
    clear: function () {
      for (var i in window.Asc.scope) delete window.Asc.scope[i];
    },
  };

  function extend(obj, plugin) {
    if (!obj || !("object" == typeof obj || "array" == typeof obj)) return obj;

    var dst = plugin === undefined ? {} : plugin;
    for (var prop in obj) {
      if (obj.hasOwnProperty(prop)) {
        dst[prop] =
          obj[prop] && "object" === typeof obj[prop]
            ? extend(obj[prop])
            : obj[prop];
      }
    }
  ...
  function getSearchParam(name) {
    ...
    if (_pos1 >= 0) {
      ...
      if (_pos2 < 0) _pos2 = _windowSearch.length;
  ...
  window.onload = function () {
    ...
    xhr.onload = function () {
      ...
      if (xhr.status === 404) return xhr.onerror();

      if (xhr.status == 200 || (xhr.status == 0 && xhr.readyState == 4)) {
        var objConfig = xhr.response;
        if (typeof objConfig == "string") objConfig = JSON.parse(objConfig);
  ...
  function onMessage(event) {
    ...
    if (window.plugin_onMessage) {
      if (!window.Asc.supportOrigins[event.origin]) return;
      window.plugin_onMessage(event);
      return;
    }

    if (!window.Asc.plugin._initInternal) return;
  ...
  window.Asc.plugin.attachEvent = function (id, action) {
    ...
    if (!pluginObj._events) pluginObj._events = {};
  ...
  window.onunload = function () {
    ...
    else window.detachEvent("onmessage", onMessage);
  };
})(window, undefined);
